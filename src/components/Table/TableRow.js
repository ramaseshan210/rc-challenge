...
const TableRow = ({data,label,setOpen,open,setInfo}) => {
  ...
  return (
      ...
      <tbody>
        ...
        <tr  className="hover:bg-green-100 cursor-pointer " onClick={()=>{setOpen(!open); setInfo(data);}} key={data.flight_number}>
          <td>
           <h1 className={style}>{data.flight_number}</h1>
          </td>
          <td>
